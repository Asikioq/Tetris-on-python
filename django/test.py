...
def register_user():
    ...
    while not user_registered:
        ...
        draw_input_box(input_box2, color2, '*' * len(text2))  # Вывод звездочек для ввода пароля
...
user_name, password = register_user()

# Подключение к базе данных
conn = sqlite3.connect('tetris_scores.db')
cursor = conn.cursor()

# Создание таблицы для лучших результатов, если её нет
cursor.execute('''
...
# Получение лучшего результата для пользователя
cursor.execute('SELECT MAX(score) FROM scores WHERE username=?', (user_name,))
best_score_result = cursor.fetchone()
best_score = best_score_result[0] if best_score_result else 0

# Закрытие соединения с базой данных
conn.close()

score = 0
filepath = load_best_score(text1)
...
cursor.execute('INSERT INTO scores (username, score) VALUES (?, ?)', (user_name, score))
conn.commit()

# Получение нового лучшего результата для пользователя
cursor.execute('SELECT MAX(score) FROM scores WHERE username=?', (user_name,))
new_best_score_result = cursor.fetchone()
new_best_score = new_best_score_result[0] if new_best_score_result else 0

# Закрытие соединения с базой данных
conn.close()

if new_best_score > best_score:
    print(f'Congratulations! New best score for user {user_name}: {new_best_score}')
else:
    print(f'Your score: {score}. Best score for user {user_name}: {best_score}')
