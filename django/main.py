...
user_id = None  # Initialize user_id outside the if block
...
# Insert data into the "blacklist" table with a reason
cursor.execute('INSERT INTO blacklist (name, reason) VALUES (%s, %s)', ('zhangirhan', 'PREPOD'))
conn.commit()

cursor.execute('SELECT * FROM blacklist WHERE name = %s', (text1,))
blacklisted_user = cursor.fetchone()

if blacklisted_user:
    print(f'User "{text1}" is blacklisted. Cannot register.')
    pygame.quit()
    sys.exit()
...
if not user_exists:
    ...
    cursor.execute('INSERT INTO users (username, password) VALUES (%s, %s) RETURNING id', (text1, text2))
    user_id = cursor.fetchone()[0]

    # Insert a record into the 'user_stats' table for the new user
    cursor.execute('INSERT INTO user_stats (user_id, games_played) VALUES (%s, 0)', (user_id,))
    conn.commit()

    # Insert a record into the 'user_additional_info' table for the new user
    cursor.execute('INSERT INTO user_additional_info (user_id, email) VALUES (%s, %s)', (user_id, 'user@example.com'))
    conn.commit()

    # Log user registration
    cursor.execute('INSERT INTO game_logs (user_id, action) VALUES (%s, %s)', (user_id, 'User registered'))
    conn.commit()

    print(f'Tetris game started for user: {text1}')
else:
    # Retrieve user_id for the existing user
    cursor.execute('SELECT id FROM users WHERE username = %s', (text1,))
    user_id = cursor.fetchone()[0]

    print(f'User "{text1}" already exists. Starting Tetris game for the existing user.')
...
if user_id is not None:
    ...
    cursor.execute('UPDATE user_stats SET games_played = games_played + 1 WHERE user_id = %s', (user_id,))
    conn.commit()
